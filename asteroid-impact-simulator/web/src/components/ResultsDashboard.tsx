      ...
      {crater && (
        <div className="card" role="region" aria-labelledby="crater-heading">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            <StatCard
              label="Diameter"
              value={`${((crater.modifiedDiameter ?? crater.diameter) / 1000).toFixed(2)} km`}
              subtitle={`${(crater.modifiedDiameter ?? crater.diameter).toFixed(0)}m`}
              color="text-orange-400"
            />
            <StatCard
              label="Depth"
              value={`${((crater.modifiedDepth ?? crater.depth) / 1000).toFixed(2)} km`}
              subtitle={`${(crater.modifiedDepth ?? crater.depth).toFixed(0)}m`}
              color="text-orange-400"
            />
            <StatCard
              label="Volume"
