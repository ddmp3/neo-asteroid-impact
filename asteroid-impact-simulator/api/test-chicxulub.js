// Debug Chicxulub crater prediction using Collins et al. (2005) formula

// Chicxulub parameters
const L = 10000;           // 10 km impactor diameter (m)
const v = 20000;           // 20 km/s velocity (m/s)
const angle = 45;          // 45° impact angle
const rho_i = 3000;        // rocky asteroid density (kg/m³)
const rho_t = 2500;        // Earth crust density (kg/m³)
const g = 9.81;            // Earth gravity (m/s²)

console.log('=== CHICXULUB TEST - Collins et al. (2005) Formula ===\n');
...
if (D_tc > 3200) {
    ...
    console.log(`\n=== VALIDATION ===`);
    console.log(`Observed Chicxulub diameter: 180 km`);
    console.log(`Predicted final diameter: ${D_final.toFixed(2)} km`);
    console.log(`Error: ${((D_final - 180) / 180 * 100).toFixed(1)}%`);
} else {
    console.log(`\nSimple crater (D_tc < 3.2 km):`);
    console.log(`  D_final = 1.25 × D_tc = ${(1.25 * D_tc / 1000).toFixed(2)} km`);
}
